...
const std::array<const IUS4V6Adapter*, 7>& AdapterTable() {
  ...
  static const std::array<const IUS4V6Adapter*, 7> kAdapters = {
      &kQwenAdapter, &kGemmaAdapter, &kBitNetAdapter, &kTernaryAdapter, &kLlamaAdapter, &kDeepSeekAdapter, &kKimiAdapter};
  return kAdapters;
}
...
const IUS4V6Adapter* FindAdapterByModel(const std::string_view modelName) {
  ...
  for (const IUS4V6Adapter* adapter : AdapterTable()) {
    ...
    if (normalized.find(family) != std::string::npos && family.size() > bestLength) {
      bestMatch = adapter;
